...
type Character = Record<{
  owner: string; // owner's principal address
  retryCount: nat64; // 배열의 랜덤값을 갱신할 수 있는 횟수
  battleHistory: Vec<string>; // 배틀 기록 아이디 배열
}>;
...
type CharacterInBattle = Record<{
  ...
  result: number;
  approved?: boolean;
  gameResults?: Vec<number>;
}>;

// 배틀 타입 정의
type Battle = Record<{
  id: string; // uuid
  characters: Vec<Character>; // 참여한 캐릭터들의 주소 배열
  results: Vec<CharacterInBattle>; // 각 캐릭터의 결과값
  betAmount: nat64; // 배팅된 토큰 수
  maxParticipantAmount: nat16; // 최대 참여자 수
  battleAdmin: string;
  winner?: CharacterInBattle;
}>;

type BattleInfo = Record<{
  id: string;
...
const characters: Vec<Character> = [];
const battles: Vec<Battle> = [];
let tokenOwner = ""; // 토큰 오너 주소

class TokenCanister extends Service {
  @serviceUpdate
  burn: (from: string, amount: nat64) => CallResult<boolean>;

  @serviceQuery
  allowanceFrom: (owner: string) => CallResult<nat64>;
...
let tokenCanisterInstance: TokenCanister;

async function _burn(from: string, amount: nat64): Promise<boolean> {
  const burnCallResult = await tokenCanisterInstance.burn(from, amount).call();

  const result = match(burnCallResult, {
...
async function _allowanceFrom(owner: string): Promise<bigint> {
  const allowanceCallResult = await tokenCanisterInstance
    .allowanceFrom(owner)
    .call();

  return match(allowanceCallResult, {
    Ok: (result) => result,
    Err: () => 0n,
  });
}
...
async function _transferFrom(
  ...
  const transferFromResponse = await tokenCanisterInstance
    .transferFrom(fromAddress, toAddress, amount)
    .call();

  return match(transferFromResponse, {
    Ok: () => true,
    Err: () => {
...
/*
 * 반환 타입이 nat16, nat32이면 무조건 내림처리 되어서 0이되고,
 * nat64는 bigint라 Math.random()의 반환타입인 Number 타입과 상충됨
 * 반환타입을 Number로 하면 아무 문제도 생기지 않음 (???)
 */
...
$update;
export function initialize(tokenCanister: string): boolean {
  ...
  tokenCanisterInstance = new TokenCanister(Principal.fromText(tokenCanister));

  return true;
}

$query;
export function getCharacterByOwner(owner: string): Character {
...
$update;
export function createCharacter(): boolean {
...
*/
$update;
export async function upgradeRetryCountWithTokens(
  amount: nat16
): Promise<boolean> {
  const caller = getCaller();
  ...
  if (burnCallResult) {
    ...
    if (indexToModify !== -1) {
      characters[indexToModify].retryCount += BigInt(retryCountAmountToUpgrade);
    }
    return true;
  } else {
    return false;
...
$query;
export function openedBattles(): Vec<BattleInfo> {
  return battles
    .filter((battle) => battle.winner === undefined)
    .map((battle) => {
      return {
        id: battle.id,
        characters: battle.characters,
        betAmount: battle.betAmount,
        maxParticipantAmount: battle.maxParticipantAmount,
      };
    });
}

$query;
export function getBattleByUuid(uuid: string): Battle {
...
*/
$update;
export async function enterBattle(battleId: string): Promise<boolean> {
  const caller = getCaller();
  const character = characters.find((char) => char.owner === caller);
  ...
  const battle = getBattleByUuid(battleId);
...
*/
$update;
export async function endBattle(battleId: string): Promise<Battle> {
  const battle = getBattleByUuid(battleId);
  ...
  for (let characterInBattle of battle.characters) {
    const character = getCharacterByOwner(characterInBattle.owner);
  ...
  for (let i = 0; i < battle.results.length; i++) {
    ...
    const character = getCharacterByOwner(battle.results[i].owner);
